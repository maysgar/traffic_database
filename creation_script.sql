...
CREATE TABLE PEOPLE(
  ...
  CONSTRAINT PEOPLE_AGE CHECK ((FLOOR((CAST (getDate() AS INTEGER) - CAST(Birth AS INTEGER)) / 365)) => 18),
  CONSTRAINT PEOPLE_AGE CHECK ((FLOOR((CAST (getDate() AS INTEGER) - CAST(Birth AS INTEGER)) / 365)) <= 120),
  --AÑADIR AL REPORT (QUITADO AGE POR REDUNDANCIA)
...
CREATE TABLE POWER_VEHICLE(
  ...
  CONSTRAINT POWER_VEHICLE_FK FOREIGN KEY (nPlate) REFERENCES VEHICLE ON DELETE CASCADE
);
...
CREATE TABLE ALLEGATION(
  ...
  */
  ...
  CONSTRAINT ALLEGATION_DRIVER_FK FOREIGN KEY (Dni) REFERENCES PEOPLE ON DELETE CASCADE, --AÑADIR AL REPORT PEOPLE EN VEZ DE DRIVES_VEHICLE
